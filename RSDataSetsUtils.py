...
class VOCLabel(object):
    ...
    def getBBoxes(self):
        with open(self.label_path, "r") as input_label_file:
            ...
            for obj in root.iter("object"):
                ...
                self.BBoxes.append(VOCBoudingBox(x_min, x_max, y_min, y_max, cls_name, is_difficult, is_truncated))
    ...
    def Convert2Yolo(self):
        ...
        for box in self.BBoxes:
            ...
            y = y*dh
            h = h*dh
            BBoxesYolo.append(YoloBoudingBox(x, y, w, h, self.clss.index(box.cls_name)))
        ...
        with open(self.label_path, "r") as input_label_file:
            ...
            for line in lines:
                for boxes in line:
                    ...
                    self.BBoxes.append(YoloBoudingBox(x_center, y_center, width, height, cls_id))

    def Convert2VOC(self):
        BBoxesVOC = []
        for box in self.BBoxes:
            xmin = int(box.center_x * self.img_w - box.bbox_width * self.img_w / 2)
            ymin = int(box.center_y * self.img_h - box.bbox_height * self.img_h / 2)
            xmax = int(box.center_x * self.img_w + ox.bbox_width * self.img_w / 2)
            ymax = int(box.center_y * self.img_h + box.bbox_height * self.img_h / 2)
            BBoxesVOC.append(VOCBoudingBox(self.img_w, self.img_h, self.img_w, box.xmin, box.xmax, box.ymin, box.ymax, clss[box.cls_id]))
...
class RSDataSetsUtils:
    ...
    @staticmethod
    def WriteVOCLabelXml(voc_label, xml_path):
    ...
    @staticmethod
    def WritrYoloLabelTxt(yolo_label, label_txt_path):
        ...
        with open(label_path, "r") as input_label_file:
            ...
            for line in lines:
                for boxes in line:
                    ...
                    y_min = min(float(box[1]), floatbox[3]), float(box[5]), float(box[7]))
    ...
    def convert2VOC(self):
        ...
        if self.dataset_type == "dota":
            for txt in txt_file:
                ...
                self.WriteVOCLabelXml(voc_label, os.path.join(save_path, output_file_name))
    ...
    def convert2Yolo(self, classes=None):
        ...
        if self.dataset_type == "dota":
            for txt in txt_file:
                ...
                if not os.path.exists(save_path):
                    os.makedirs(save_path)
                self.WritrYoloLabelTxt(yolo_label, os.path.join(save_path, output_file_name))
        elif self.dataset_type == "visdrone":
            for txt in txt_file:
                txt_full_path = os.path.join(self.labels_folder_path, txt)
...
class RSDataSetsSpliter(RSDataSetsUtils):
    ...
    def image_split(self, img_dst_path, size_w, size_h, step):
    ...
    def get_split_voc_label(self, oringin_voc_label, out_img_floder, out_txt_floder, start_x, start_y, size_w, size_h):
        ...
        label_name = os.path.join(out_txt_floder, label_name[0] + "_%s_%s" %(start_y, start_x) + label_name[1])
        voc_label = VOCLabel(img_name, label_name, oringin_voc_label.clss)
    ...
    def txt_split(self, img_floder, out_img_floder, txt_floder, out_txt_floder, size_h, size_w):
        if not os.path.exists(out_txt_floder):
            os.makedirs(out_txt_floder)
        classes = ["plane", "ship", "storage-tank", "baseball-diamond", "tennis-court", "basketball-court", "ground-track-field", "harbor", "bridge", "large-vehicle", "small-vehicle", "helicopter", "roundabout", "soccer-ball-field", "swimming-pool", "container-crane", "airport", "helipad"]
        ...
        for img_name in img_list:
            ...
            oringin_voc_label.getBBoxes()
            voc_label = self.get_split_voc_label(oringin_voc_label, out_img_floder, out_txt_floder, x, y, size_w, size_h)
            self.WriteVOCLabelXml(voc_label, os.path.join(out_txt_floder, name + ".xml"))
